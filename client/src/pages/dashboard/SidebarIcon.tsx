...
const SidebarIcons = (props: Props) => {
    const { state, actions } = useStateMachine({ updateSidebarState })

    return (
        <div className="w-[50px] z-[60]">
            <nav
                className={`fixed bottom-0 top-0 left-0 bg-app-violet w-[60px] 
             pt-32 lg:pt-48`}
            >
                <ul className="relative flex flex-col items-center flex-1 lg:items-center gap-y-16 text-white w-full pr-4 ">
                    {props.role === 'EMPLOYER' && (
                        <li
                            className={`
        ...
        `}
                ...
                {state.sidebarState?.status === 0 && (
                    <div className="hidden md:block md:fixed bottom-10 md:bottom-0 lg:left-4">
                        <button
                            type="button"
                            onClick={() => {
                                console.log('fdsfdsfdsfdsfsfdsfsdfdsfds')
                                actions.updateSidebarState(1)
                            }}
                        >
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
